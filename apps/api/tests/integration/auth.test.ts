...
describe("Auth Integration Tests", () => {
  ...
  it("logs in successfully and returns JWT + HttpOnly refresh cookie", async () => {
    ...
    const refreshCookie = Array.isArray(cookies) ? cookies.find((c) => c.startsWith("refresh_token=")) : cookies;
  ...
  it("rotates refresh token and detects token theft reuse", async () => {
    ...
    const cookie1 = Array.isArray(cookies) ? cookies.find((c) => c.startsWith("refresh_token=")) : cookies;
    ...
    const cookie2 = Array.isArray(cookies2) ? cookies2.find((c) => c.startsWith("refresh_token=")) : cookies2;
  ...
  it("logs out user and clears cookie", async () => {
    ...
    const cookies = loginRes.headers["set-cookie"];
    const cookie = Array.isArray(cookies) ? cookies.find((c) => c.startsWith("refresh_token=")) : cookies;
